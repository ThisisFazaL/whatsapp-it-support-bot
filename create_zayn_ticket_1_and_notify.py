...
from app.database import (
    ...
    SupportAdmin, Employee, Category, Subcategory, IssueType, Priority, Location
)
...
async def create_zayn_ticket_and_notify():
    ...
    async with async_session_factory() as session:
        ...
        # 2. Fetch Category, Subcategory, IssueType, Location
        ...
        loc_res = await session.execute(select(Location).where(Location.location_name.ilike("%Tagoneswa%")))
        ...
        emp_name = t_obj.employee.full_name if t_obj.employee else "Zayn"
        emp_phone = t_obj.employee.phone if t_obj.employee else "263713866223"
        ...
        issue_name = t_obj.issue_type.issue_name if t_obj.issue_type else "Door hinges squeaking or misaligned"
        priority_name = t_obj.priority.priority_name if t_obj.priority else "Medium"
        room_area = t_obj.room_area
